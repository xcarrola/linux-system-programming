...
consumer(void *arg)
{
    ...
    while (!consDone) {
        ...
        /* Wait for something to consume.
           ...
           We should wait on the condition variable only if there are
           no units available *and* units are still being produced
           (unitsConsumed < totalUnits), otherwise we might wait and
           never be woken up. */
...
main(int argc, char *argv[])
{
    if (argc < 4) {
        fprintf(stderr,
                "Usage: %s num-prod num-cons num-units [sleep-factor]\n\n",
                argv[0]);
    ...
    for (int j = 0; j < numProd; j++) {
        int s = pthread_create(&tid, NULL, producer, NULL);
        if (s != 0)
    ...
    for (int j = 0; j < numCons; j++) {
        int s = pthread_create(&tid, NULL, consumer, NULL);
        if (s != 0)
            errExitEN(s, "pthread_create");
    }
